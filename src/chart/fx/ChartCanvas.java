...
public class ChartCanvas extends Canvas implements ChartChangeListener, OverlayChangeListener {
	...
	public ChartCanvas(JFreeChart chart) {
		...
		this.chart_Mouse_Listeners = new ArrayList<>();
		...
		this.available_Mouse_Handlers = new ArrayList<>();

		this.auxiliary_Mouse_Handlers = new ArrayList<>();
	...
	public MouseHandlerFX getMouseHandler(String id) {
		...
		return this.auxiliary_Mouse_Handlers.stream().filter(h -> h.getID().equals(id)).findFirst().orElse(null);
	}
	...
	private boolean hasUniqueID(MouseHandlerFX handler) {
		...
		return this.auxiliary_Mouse_Handlers.stream().filter(h -> handler.getID().equals(h.getID())).findFirst()
				.map(h -> false).orElse(true);
	}
	...
	public final void draw() {
		...
		ctx.restore();
		this.overlays.forEach(overlay -> overlay.paintOverlay(g2, this));
		this.anchor = null;
	}
	...
	private void handleMousePressed(MouseEvent e) {
		if (this.live_Handler == null) {
			this.available_Mouse_Handlers.stream()
					.filter(handler -> handler.isEnabled() && handler.hasMatchingModifiers(e))
					.forEach(handler -> this.live_Handler = handler);
		}
		...
		this.auxiliary_Mouse_Handlers.stream().filter(MouseHandlerFX::isEnabled)
				.forEach(handler -> handler.handleMousePressed(this, e));
	}
	...
	private void handleMouseMoved(MouseEvent e) {
		...
		this.auxiliary_Mouse_Handlers.stream().filter(MouseHandlerFX::isEnabled)
				.forEach(handler -> handler.handleMouseMoved(this, e));
	}
	...
	private void handleMouseDragged(MouseEvent e) {
		...
		// pass on the event to the auxiliary handlers
		this.auxiliary_Mouse_Handlers.stream().filter(MouseHandlerFX::isEnabled)
				.forEach(handler -> handler.handleMouseDragged(this, e));
	}
	...
	private void handleMouseReleased(MouseEvent e) {
		...
		// pass on the event to the auxiliary handlers
		this.auxiliary_Mouse_Handlers.stream().filter(MouseHandlerFX::isEnabled)
				.forEach(handler -> handler.handleMouseReleased(this, e));
	}
	...
	private void handleMouseClicked(MouseEvent e) {
		...
		// pass on the event to the auxiliary handlers
		this.auxiliary_Mouse_Handlers.stream().filter(MouseHandlerFX::isEnabled)
				.forEach(handler -> handler.handleMouseClicked(this, e));
	}
	...
	protected void handleScroll(ScrollEvent e) {
		...
		this.auxiliary_Mouse_Handlers.stream().filter(MouseHandlerFX::isEnabled)
				.forEach(handler -> handler.handleScroll(this, e));
	}
	...
	public void dispatchMouseMovedEvent(Point2D point, MouseEvent e) {
		...
		this.chart_Mouse_Listeners.forEach(listener -> listener.chartMouseMoved(event));
	}
	...
	public void dispatchMouseClickedEvent(Point2D point, MouseEvent e) {
		...
		ChartEntity entity = this.info.getEntityCollection().getEntity(x, y);
		ChartMouseEventFX event = new ChartMouseEventFX(this.chart, e, entity);
		this.chart_Mouse_Listeners.forEach(listener -> listener.chartMouseClicked(event));
	}
}
