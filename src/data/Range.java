...
public strictfp class Range implements Serializable {
	...
	/**
	 * Returns {@code true} if the range intersects with the specified range, and
	 * {@code false} otherwise.
	 *
	 ...
	 * Note that:
	 * <ul>
	 * <li>either range can be {@code null}, in which case the other range is
	 * returned;</li>
	...
	/**
	 * Returns a new range that spans both {@code range1} and {@code range2}. This
	 * method has a special handling to ignore Double.NaN values.
	 *
	 * @param range1 the first range ({@code null} permitted).
	 * @param range2 the second range ({@code null} permitted).
	 *
	...
	/**
	 * Returns a range that includes all the values in the specified {@code range}
	 * AND the specified {@code value}.
	 *
	 ...
	 * @param base              the base range ({@code null} not permitted).
	 * @param delta             the shift amount.
	 * @param allowZeroCrossing a flag that determines whether or not the bounds of
	 *                          the range are allowed to cross zero after
	 *                          adjustment.
	 *
	 * @return A new range.
	 */
	...
	/**
	 * Returns the given {@code value} adjusted by {@code delta} but with a check to
	 * prevent the result from crossing {@code 0.0}.
	 *
	 * @param value the value.
