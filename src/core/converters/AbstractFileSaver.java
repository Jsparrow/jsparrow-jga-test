...
import core.EnvironmentHandler;
import core.Option;
...
public abstract class AbstractFileSaver extends AbstractSaver
	...
	/** The writer. */
	private transient BufferedWriter mWriter;
			...
			try {
				...
				if (success) {
					if (m_useRelativePath) {
						try {
							mOutputFile = Utils.convertToRelativePath(file);
						} catch (Exception e) {
							mOutputFile = file;
						}
					} else {
					...
					setDestination(new FileOutputStream(mOutputFile));
				}
			} catch (Exception ex) {
				throw new IOException(
						"Cannot create a new output file (Reason: " + ex.toString() + "). Standard out is used.");
			} finally {
				if (!success) {
					System.err.println("Cannot create a new output file. Standard out is used.");
					mOutputFile = null; // use standard out
				}
			}
		}
	}
