...
public class Option implements RevisionHandler {
	...
	public static Vector<Option> listOptionsForClass(Class<?> clazz) {
		Vector<Option> results = new Vector<>();
					...
					if (value != null) {
						...
						if (value.getClass().isArray()) {
							...
							if (((Object[]) value).length == 0) {
								// remove the initial option identifier (added
								// above)
						...
						if (value != null && processOpt) {
							if (value.getClass().isArray() && ((Object[]) value).length >= 0) {
								...
								if (tagIndex != Integer.MAX_VALUE) {
									...
									throw new Exception(
											"Unable to set option: '" + parameterDescription.commandLineParamName()
								...
								valueToSet = EnumHelper.valueFromString(helper.getEnumClass(), optionValue);
							} else if (value instanceof OptionHandler) {
								valueToSet = constructOptionHandlerValue(optionValue);
							} else if (value instanceof Number) {
								try {
									if (value instanceof Integer) {
										valueToSet = Integer.valueOf(optionValue);
									} else if (value instanceof Long) {
										valueToSet = Long.valueOf(optionValue);
									} else if (value instanceof Double) {
										valueToSet = Double.valueOf(optionValue);
									} else if (value instanceof Float) {
										valueToSet = Float.valueOf(optionValue);
									}
								} catch (NumberFormatException e) {
									throw new Exception("Option: '" + parameterDescription.commandLineParamName()
